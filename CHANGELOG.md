...
## 25.09.2026 — Таймаут доставки лида в Telegram (устойчивость на прод-сервере)

**Что было.** После деплоя на прод-VPS (`/var/www/pinpiece`, pm2, порт 3002) в логах пошло `[lead] telegram delivery failed: ETIMEDOUT` на каждую заявку. Причина: `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` в `.env` заданы, но сервер не может достучаться до `api.telegram.org` (типовая блокировка Telegram на российских серверах без прокси). На пользователя это не влияет — основной канал заявки открывается на клиенте (`t.me/pinpiece` с готовым текстом), а `/api/lead` — фоновый резерв. Технический минус: `fetch` без таймаута висел до системного TCP-таймаута на каждый адрес (IPv6+IPv4), копя зависшие соединения.

**Что сделано.**
- `app/api/lead/route.ts` — к `fetch` в Telegram добавлен `signal: AbortSignal.timeout(5000)`. При недоступности/ошибке HTTP лид не теряется: пишется в лог сервера (`[lead] received: …`) как запасной канал. Логи стали компактными (одна строка с причиной вместо стектрейса).

**Как проверено.**
- `next build` — успешно, 10 маршрутов, `/api/lead` динамический.
- На прод-сервере: `curl -sI /` и `/privacy` → `HTTP/1.1 200 OK`, счётчик Метрики `mc.yandex.ru/metrika/tag.js?id=112993063` в серверном HTML, pm2 `pinpiece` — `online` на Next 16.3.6.
